...
@Component({
  selector: 'app-edit-house-design',
  templateUrl: './edit-house-design.page.html',
  styleUrls: ['./edit-house-design.page.scss'],
})
export class EditHouseDesignPage implements OnInit, AfterViewInit, OnDestroy {
  houseForm: FormGroup;
  ...
  ngOnInit() {
    ...
    this.houseService
      ...
      .subscribe((house) => {
        this.house = house;
        this.initForm(house);
        house.items.forEach((houseItem) => {
          this.addHouseItemToCanvas(houseItem);
        });
      });
    this.getCurrentUser();
  }
  ...
  async createHouse() {
    if (this.app.stage.children.length && this.houseForm.valid) {
      const thumbnailURL = await this.houseService.uploadSketchThumbnail(
        document
          .querySelector('#editHouseContainer')
      ...
      this.houseService.update(house).then(async () => {
  ...
  private initCanvas() {
    ...
    const canvasContainer = document.querySelector('#editHouseContainer');
  ...
  private addHouseItemToCanvas(houseItem: HouseItem, count = 1) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (houseItem.position.x === 0 && houseItem.position.y === 0) {
        item.x = this.app.screen.width / 2;
        item.y = this.app.screen.height / 2;
      } else {
        item.x = houseItem.position.x;
        item.y = houseItem.position.y;
      }
